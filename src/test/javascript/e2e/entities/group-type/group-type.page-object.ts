...
export class GroupTypeComponentsPage {
  createButton = element(by.id('jh-create-entity'));
  deleteButtons = element.all(by.css('jhi-group-type div table .btn-danger'));
  title = element.all(by.css('jhi-group-type div h2#page-heading span')).first();
...
export class GroupTypeUpdatePage {
  pageTitle = element(by.id('jhi-group-type-heading'));
  saveButton = element(by.id('save-entity'));
  cancelButton = element(by.id('cancel-save'));

  groupTypeIdInput = element(by.id('field_groupTypeId'));
  ...
  async setGroupTypeIdInput(groupTypeId: string): Promise<void> {
    await this.groupTypeIdInput.sendKeys(groupTypeId);
  }

  async getGroupTypeIdInput(): Promise<string> {
    return await this.groupTypeIdInput.getAttribute('value');
  }
...
export class GroupTypeDeleteDialog {
  private dialogTitle = element(by.id('jhi-delete-groupType-heading'));
  private confirmButton = element(by.id('jhi-confirm-delete-groupType'));
