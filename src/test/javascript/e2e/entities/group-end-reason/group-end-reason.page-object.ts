...
export class GroupEndReasonComponentsPage {
  createButton = element(by.id('jh-create-entity'));
  deleteButtons = element.all(by.css('jhi-group-end-reason div table .btn-danger'));
  title = element.all(by.css('jhi-group-end-reason div h2#page-heading span')).first();
...
export class GroupEndReasonUpdatePage {
  pageTitle = element(by.id('jhi-group-end-reason-heading'));
  saveButton = element(by.id('save-entity'));
  cancelButton = element(by.id('cancel-save'));

  endReasonCodeInput = element(by.id('field_endReasonCode'));
  endReasonInput = element(by.id('field_endReason'));
  ...
  async setEndReasonInput(endReason: string): Promise<void> {
    await this.endReasonInput.sendKeys(endReason);
  }

  async getEndReasonInput(): Promise<string> {
    return await this.endReasonInput.getAttribute('value');
  }
...
export class GroupEndReasonDeleteDialog {
  private dialogTitle = element(by.id('jhi-delete-groupEndReason-heading'));
  private confirmButton = element(by.id('jhi-confirm-delete-groupEndReason'));
