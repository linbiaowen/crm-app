...
/**
 * Performance test for the ProductAttributes entity.
 */
class ProductAttributesGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the ProductAttributes entity")
        ...
        .repeat(2) {
            ...
            .exec(http("Create new productAttributes")
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_productAttributes_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created productAttributes")
                .get("${new_productAttributes_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created productAttributes")
            .delete("${new_productAttributes_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
