...
/**
 * Performance test for the SupremeMaster entity.
 */
class SupremeMasterGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the SupremeMaster entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all supremeMasters")
            .get("/api/supreme-masters")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new supremeMaster")
            .post("/api/supreme-masters")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "supremeSeqId":null
                ...
                , "peCode":"SAMPLE_TEXT"
                , "personalExecDirectLine":"SAMPLE_TEXT"
                , "personalExecName":"SAMPLE_TEXT"
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_supremeMaster_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created supremeMaster")
                .get("${new_supremeMaster_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created supremeMaster")
            .delete("${new_supremeMaster_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
