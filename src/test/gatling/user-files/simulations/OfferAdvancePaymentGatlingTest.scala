...
/**
 * Performance test for the OfferAdvancePayment entity.
 */
class OfferAdvancePaymentGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the OfferAdvancePayment entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all offerAdvancePayments")
            .get("/api/offer-advance-payments")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new offerAdvancePayment")
            .post("/api/offer-advance-payments")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_offerAdvancePayment_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created offerAdvancePayment")
                .get("${new_offerAdvancePayment_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created offerAdvancePayment")
            .delete("${new_offerAdvancePayment_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
