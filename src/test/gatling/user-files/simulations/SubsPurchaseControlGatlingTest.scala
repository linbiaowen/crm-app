...
/**
 * Performance test for the SubsPurchaseControl entity.
 */
class SubsPurchaseControlGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the SubsPurchaseControl entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all subsPurchaseControls")
            .get("/api/subs-purchase-controls")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new subsPurchaseControl")
            .post("/api/subs-purchase-controls")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "subscriptionId":"SAMPLE_TEXT"
                , "serviceType":"SAMPLE_TEXT"
                , "purchaseControlFlag":"SAMPLE_TEXT"
                , "status":"INACTIVE"
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_subsPurchaseControl_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created subsPurchaseControl")
                .get("${new_subsPurchaseControl_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created subsPurchaseControl")
            .delete("${new_subsPurchaseControl_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
