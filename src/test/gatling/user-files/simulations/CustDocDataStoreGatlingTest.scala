...
/**
 * Performance test for the CustDocDataStore entity.
 */
class CustDocDataStoreGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the CustDocDataStore entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all custDocDataStores")
            .get("/api/cust-doc-data-stores")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new custDocDataStore")
            .post("/api/cust-doc-data-stores")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "docDataStoreId":null
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_custDocDataStore_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created custDocDataStore")
                .get("${new_custDocDataStore_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created custDocDataStore")
            .delete("${new_custDocDataStore_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
