...
/**
 * Performance test for the CustAcctBlackList entity.
 */
class CustAcctBlackListGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the CustAcctBlackList entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all custAcctBlackLists")
            .get("/api/cust-acct-black-lists")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new custAcctBlackList")
            .post("/api/cust-acct-black-lists")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "blackListId":null
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_custAcctBlackList_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created custAcctBlackList")
                .get("${new_custAcctBlackList_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created custAcctBlackList")
            .delete("${new_custAcctBlackList_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
