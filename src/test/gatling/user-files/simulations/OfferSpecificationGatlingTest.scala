...
/**
 * Performance test for the OfferSpecification entity.
 */
class OfferSpecificationGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the OfferSpecification entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all offerSpecifications")
            .get("/api/offer-specifications")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new offerSpecification")
            .post("/api/offer-specifications")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "offerSpecId":null
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_offerSpecification_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created offerSpecification")
                .get("${new_offerSpecification_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created offerSpecification")
            .delete("${new_offerSpecification_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
