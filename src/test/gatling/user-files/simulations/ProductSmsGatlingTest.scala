...
/**
 * Performance test for the ProductSms entity.
 */
class ProductSmsGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the ProductSms entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all productSms")
            .get("/api/product-sms")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new productSms")
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_productSms_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created productSms")
                .get("${new_productSms_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created productSms")
            .delete("${new_productSms_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
