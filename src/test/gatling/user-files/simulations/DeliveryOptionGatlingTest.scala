...
/**
 * Performance test for the DeliveryOption entity.
 */
class DeliveryOptionGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the DeliveryOption entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all deliveryOptions")
            .get("/api/delivery-options")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new deliveryOption")
            .post("/api/delivery-options")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "deliveryOption":"EFLOCKER"
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_deliveryOption_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created deliveryOption")
                .get("${new_deliveryOption_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created deliveryOption")
            .delete("${new_deliveryOption_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
