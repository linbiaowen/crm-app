...
/**
 * Performance test for the DeliveryMethod entity.
 */
class DeliveryMethodGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the DeliveryMethod entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all deliveryMethods")
            .get("/api/delivery-methods")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new deliveryMethod")
            .post("/api/delivery-methods")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "deliveryMethodId":null
                , "deliveryMethod":"SAMPLE_TEXT"
                , "deliveryMethodDesc":"SAMPLE_TEXT"
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_deliveryMethod_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created deliveryMethod")
                .get("${new_deliveryMethod_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created deliveryMethod")
            .delete("${new_deliveryMethod_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
