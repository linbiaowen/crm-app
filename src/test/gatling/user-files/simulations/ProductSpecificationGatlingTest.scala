...
/**
 * Performance test for the ProductSpecification entity.
 */
class ProductSpecificationGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the ProductSpecification entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all productSpecifications")
            .get("/api/product-specifications")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new productSpecification")
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_productSpecification_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created productSpecification")
                .get("${new_productSpecification_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created productSpecification")
            .delete("${new_productSpecification_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
