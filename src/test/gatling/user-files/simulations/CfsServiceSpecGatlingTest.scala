...
/**
 * Performance test for the CfsServiceSpec entity.
 */
class CfsServiceSpecGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the CfsServiceSpec entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all cfsServiceSpecs")
            .get("/api/cfs-service-specs")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new cfsServiceSpec")
            .post("/api/cfs-service-specs")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "serviceSpecId":"SAMPLE_TEXT"
                , "serviceSpecDesc":"SAMPLE_TEXT"
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_cfsServiceSpec_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created cfsServiceSpec")
                .get("${new_cfsServiceSpec_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created cfsServiceSpec")
            .delete("${new_cfsServiceSpec_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
