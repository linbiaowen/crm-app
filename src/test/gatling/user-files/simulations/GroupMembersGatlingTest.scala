...
/**
 * Performance test for the GroupMembers entity.
 */
class GroupMembersGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the GroupMembers entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all groupMembers")
            .get("/api/group-members")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new groupMembers")
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_groupMembers_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created groupMembers")
                .get("${new_groupMembers_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created groupMembers")
            .delete("${new_groupMembers_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
