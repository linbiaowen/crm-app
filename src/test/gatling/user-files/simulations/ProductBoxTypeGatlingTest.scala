...
/**
 * Performance test for the ProductBoxType entity.
 */
class ProductBoxTypeGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the ProductBoxType entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all productBoxTypes")
            .get("/api/product-box-types")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new productBoxType")
            .post("/api/product-box-types")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "boxType":"SAMPLE_TEXT"
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_productBoxType_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created productBoxType")
                .get("${new_productBoxType_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created productBoxType")
            .delete("${new_productBoxType_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
