...
/**
 * Performance test for the BlackListMaster entity.
 */
class BlackListMasterGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the BlackListMaster entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all blackListMasters")
            .get("/api/black-list-masters")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new blackListMaster")
            .post("/api/black-list-masters")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "blackListCode":"SAMPLE_TEXT"
                , "blackListReason":"SAMPLE_TEXT"
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_blackListMaster_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created blackListMaster")
                .get("${new_blackListMaster_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created blackListMaster")
            .delete("${new_blackListMaster_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
