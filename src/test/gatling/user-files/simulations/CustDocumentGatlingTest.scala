...
/**
 * Performance test for the CustDocument entity.
 */
class CustDocumentGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the CustDocument entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all custDocuments")
            .get("/api/cust-documents")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new custDocument")
            .post("/api/cust-documents")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "custDocId":"SAMPLE_TEXT"
                , "custAcctId":"SAMPLE_TEXT"
                , "subscriptionId":"SAMPLE_TEXT"
                , "docType":"HKID"
                , "docIdNumber":"SAMPLE_TEXT"
                , "docDataStoreId":null
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_custDocument_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created custDocument")
                .get("${new_custDocument_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created custDocument")
            .delete("${new_custDocument_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
