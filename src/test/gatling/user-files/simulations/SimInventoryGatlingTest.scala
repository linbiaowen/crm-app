...
/**
 * Performance test for the SimInventory entity.
 */
class SimInventoryGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the SimInventory entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all simInventories")
            .get("/api/sim-inventories")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new simInventory")
            .post("/api/sim-inventories")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "iccid":"SAMPLE_TEXT"
                ...
                , "status":"INACTIVE"
                , "offerValidFromDate":"SAMPLE_TEXT"
                , "offerValidToDate":"SAMPLE_TEXT"
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_simInventory_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created simInventory")
                .get("${new_simInventory_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created simInventory")
            .delete("${new_simInventory_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
