...
/**
 * Performance test for the SubscriptionGroup entity.
 */
class SubscriptionGroupGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the SubscriptionGroup entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all subscriptionGroups")
            .get("/api/subscription-groups")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new subscriptionGroup")
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_subscriptionGroup_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created subscriptionGroup")
                .get("${new_subscriptionGroup_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created subscriptionGroup")
            .delete("${new_subscriptionGroup_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
