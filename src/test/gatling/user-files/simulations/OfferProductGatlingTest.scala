...
/**
 * Performance test for the OfferProduct entity.
 */
class OfferProductGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the OfferProduct entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all offerProducts")
            .get("/api/offer-products")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new offerProduct")
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_offerProduct_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created offerProduct")
                .get("${new_offerProduct_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created offerProduct")
            .delete("${new_offerProduct_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
