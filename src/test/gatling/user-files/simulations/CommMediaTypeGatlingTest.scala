...
/**
 * Performance test for the CommMediaType entity.
 */
class CommMediaTypeGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the CommMediaType entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all commMediaTypes")
            .get("/api/comm-media-types")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new commMediaType")
            .post("/api/comm-media-types")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "optoutMediaId":"SAMPLE_TEXT"
                , "mediaType":"SAMPLE_TEXT"
                , "mediaTypeDesc":"SAMPLE_TEXT"
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_commMediaType_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created commMediaType")
                .get("${new_commMediaType_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created commMediaType")
            .delete("${new_commMediaType_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
