...
/**
 * Performance test for the GroupEndReason entity.
 */
class GroupEndReasonGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the GroupEndReason entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all groupEndReasons")
            .get("/api/group-end-reasons")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new groupEndReason")
            .post("/api/group-end-reasons")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "endReasonCode":"SAMPLE_TEXT"
                , "endReason":"SAMPLE_TEXT"
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_groupEndReason_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created groupEndReason")
                .get("${new_groupEndReason_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created groupEndReason")
            .delete("${new_groupEndReason_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
