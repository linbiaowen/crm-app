...
/**
 * Performance test for the BillCycle entity.
 */
class BillCycleGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the BillCycle entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all billCycles")
            .get("/api/bill-cycles")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new billCycle")
            .post("/api/bill-cycles")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "billCycleId":"0"
                , "billCycleDesc":"SAMPLE_TEXT"
                , "billCycle":"0"
                , "billFrequency":"SAMPLE_TEXT"
                , "billCycleStartDate":"2020-01-01T00:00:00.000Z"
                , "billCycleEndDate":"2020-01-01T00:00:00.000Z"
                , "dueDateOffset":"0"
                , "directDebitProcessDay":"0"
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_billCycle_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created billCycle")
                .get("${new_billCycle_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created billCycle")
            .delete("${new_billCycle_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
