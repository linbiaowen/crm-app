...
/**
 * Performance test for the CustAddress entity.
 */
class CustAddressGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the CustAddress entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all custAddresses")
            .get("/api/cust-addresses")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new custAddress")
            .post("/api/cust-addresses")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "addressId":null
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_custAddress_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created custAddress")
                .get("${new_custAddress_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created custAddress")
            .delete("${new_custAddress_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
