...
/**
 * Performance test for the CommOptoutType entity.
 */
class CommOptoutTypeGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the CommOptoutType entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all commOptoutTypes")
            .get("/api/comm-optout-types")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new commOptoutType")
            .post("/api/comm-optout-types")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "optoutTypeId":"SAMPLE_TEXT"
                , "optoutType":"SAMPLE_TEXT"
                , "optoutTypeDesc":"SAMPLE_TEXT"
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_commOptoutType_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created commOptoutType")
                .get("${new_commOptoutType_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created commOptoutType")
            .delete("${new_commOptoutType_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
