...
/**
 * Performance test for the OfferDiscount entity.
 */
class OfferDiscountGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the OfferDiscount entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all offerDiscounts")
            .get("/api/offer-discounts")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new offerDiscount")
            .post("/api/offer-discounts")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_offerDiscount_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created offerDiscount")
                .get("${new_offerDiscount_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created offerDiscount")
            .delete("${new_offerDiscount_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
