...
/**
 * Performance test for the OfferPromotion entity.
 */
class OfferPromotionGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the OfferPromotion entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all offerPromotions")
            .get("/api/offer-promotions")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new offerPromotion")
            .post("/api/offer-promotions")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                ...
                , "promoType":"SAMPLE_TEXT"
                , "offerPrice":"0"
                , "offerDiscount":"0"
                , "freeDataOfferId":"SAMPLE_TEXT"
                , "status":"INACTIVE"
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_offerPromotion_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created offerPromotion")
                .get("${new_offerPromotion_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created offerPromotion")
            .delete("${new_offerPromotion_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
