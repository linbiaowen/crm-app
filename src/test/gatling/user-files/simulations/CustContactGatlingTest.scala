...
/**
 * Performance test for the CustContact entity.
 */
class CustContactGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the CustContact entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all custContacts")
            .get("/api/cust-contacts")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new custContact")
            .post("/api/cust-contacts")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "contactId":null
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_custContact_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created custContact")
                .get("${new_custContact_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created custContact")
            .delete("${new_custContact_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
