...
/**
 * Performance test for the ProductVoice entity.
 */
class ProductVoiceGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the ProductVoice entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all productVoices")
            .get("/api/product-voices")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new productVoice")
            .post("/api/product-voices")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "voiceId":"SAMPLE_TEXT"
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_productVoice_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created productVoice")
                .get("${new_productVoice_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created productVoice")
            .delete("${new_productVoice_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
