...
/**
 * Performance test for the CfsServices entity.
 */
class CfsServicesGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the CfsServices entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all cfsServices")
            .get("/api/cfs-services")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new cfsServices")
            .post("/api/cfs-services")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_cfsServices_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created cfsServices")
                .get("${new_cfsServices_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created cfsServices")
            .delete("${new_cfsServices_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
