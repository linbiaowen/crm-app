...
/**
 * Performance test for the SubscriptionProduct entity.
 */
class SubscriptionProductGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the SubscriptionProduct entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all subscriptionProducts")
            .get("/api/subscription-products")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new subscriptionProduct")
            .post("/api/subscription-products")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "productSubsriptionSeqId":null
                , "orderId":"SAMPLE_TEXT"
                ...
                , "secondMsisdn":"SAMPLE_TEXT"
                , "secondImsi":"SAMPLE_TEXT"
                , "quantity":"0"
                , "terminationReasonCode":"SAMPLE_TEXT"
                , "offerId":"SAMPLE_TEXT"
                , "offerName":"SAMPLE_TEXT"
                , "offerType":"SAMPLE_TEXT"
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_subscriptionProduct_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created subscriptionProduct")
                .get("${new_subscriptionProduct_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created subscriptionProduct")
            .delete("${new_subscriptionProduct_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
