...
/**
 * Performance test for the ModelCategory entity.
 */
class ModelCategoryGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the ModelCategory entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all modelCategories")
            .get("/api/model-categories")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new modelCategory")
            .post("/api/model-categories")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "modelCate":"SAMPLE_TEXT"
                , "parentModelCate":"SAMPLE_TEXT"
                , "modelCateDesc":"SAMPLE_TEXT"
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_modelCategory_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created modelCategory")
                .get("${new_modelCategory_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created modelCategory")
            .delete("${new_modelCategory_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
