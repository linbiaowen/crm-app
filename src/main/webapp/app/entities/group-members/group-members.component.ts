...
export class GroupMembersComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number): void {
    ...
    this.groupMembersService
      ...
      .subscribe(
        (res: HttpResponse<IGroupMembers[]>) => this.onSuccess(res.body, res.headers, pageToLoad),
        () => this.onError()
      );
  }
  ...
  trackId(index: number, item: IGroupMembers): string {
    // eslint-disable-next-line @typescript-eslint/no-unnecessary-type-assertion
    return item.id!;
  }

  registerChangeInGroupMembers(): void {
    this.eventSubscriber = this.eventManager.subscribe('groupMembersListModification', () => this.loadPage());
  }

  delete(groupMembers: IGroupMembers): void {
    const modalRef = this.modalService.open(GroupMembersDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
  ...
  protected onSuccess(data: IGroupMembers[] | null, headers: HttpHeaders, page: number): void {
